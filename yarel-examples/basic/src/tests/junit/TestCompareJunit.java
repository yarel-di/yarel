...
public class TestCompareJunit {
	...
	public static final BigInteger[][] DATASET_LESS = List.of( //
			new int[][] { // i j expected (if j > i)
					new int[] { 0, 0, 0 }, //
	...
	).stream() // end of "List.of(", start streaming those arrays
			.map(TestALL::registersFrom) //
	...
	public static boolean test(IntCompTest itc) {
		...
		if (IntCompTest.Less == itc || IntCompTest.More == itc) {
			boolean isLess = IntCompTest.Less == itc;
			for (BigInteger[] data : DATASET_LESS) {
