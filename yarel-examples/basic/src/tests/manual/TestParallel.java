...
public class TestParallel {
	...
	public static void testParallelFirstMethods() {
		BigInteger[] data = TestALL.registersFrom(new int[] { 10, 10, 10, 10, 10, 10, 10 });
		...
		BigInteger[] lpData = new BigInteger[12];
		for (int i = 0; i < lpData.length; i++) {
			lpData[i] = BigInteger.valueOf(i + 1);
		}
		lpData[10] = BigInteger.valueOf(-77);
	...
	public static void testToOdd() {
		...
		System.out.println("\n\n-----------------------------------\ntest TO ODD\n-------------------------");
		for (int tt : toTest) {
			a = TestALL.registersFrom(new int[] { tt, 0 });
			System.out.println("\ntesting: " + tt);
			toOdd.b(a);
	...
	public static void testTriIncPar() {
		...
		for (int tt : toTest) {
			a = TestALL.registersFrom(new int[] { tt, 1000 + tt, tt << 1 });
	...
	public static void testUpAndDown() {
		...
		System.out.println("\n\n-----------------------------------\ntest UP AND DOWN\n-------------------------");
		for (int tt : toTest) {
			a = TestALL.registersFrom(new int[] { tt, 1000 + tt, tt << 1, 0 });
			System.out.println("\ntesting: " + tt + " -> " + Arrays.toString(a));
			triIncPar.b(a);
			System.out.println("-> got -> " + Arrays.toString(a));
			triIncParInv.b(a);
