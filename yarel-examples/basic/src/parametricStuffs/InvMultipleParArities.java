...
public class InvMultipleParArities implements RPP {
	public InvMultipleParArities(//arities:
		int IncrAr, int NegAr
		,
		int IncrAmount,
		int NegAmount
		){
		this.__fixedRegistersAmount__ = 0;
		if(IncrAr < 0){ throw new WrongArityException("The arity \"IncrAr\" cannot be negative: " + IncrAr); }
		this.IncrAr = IncrAr;
		if(NegAr < 0){ throw new WrongArityException("The arity \"NegAr\" cannot be negative: " + NegAr); }
		this.NegAr = NegAr;
		// if(IncrAmount < 0){ throw new WrongArityException("The parameter \"IncrAmount\" cannot be negative: " + IncrAmount); }
		this.IncrAmount = IncrAmount;
		
		// if(NegAmount < 0){ throw new WrongArityException("The parameter \"NegAmount\" cannot be negative: " + NegAmount); }
		this.NegAmount = NegAmount;
		
	}
	protected InvMultipleParArities(){
		this(1, 1,0, 0);
	}
	
	protected final int __fixedRegistersAmount__;
	protected final int IncrAr;
	protected final int NegAr;
	protected final int IncrAmount;
	protected final int NegAmount;
	
	
	public int getFixedRegistersAmount(){
		return this.__fixedRegistersAmount__;
	}
	public int getIncrAr(){ return this.IncrAr; }
	public int getNegAr(){ return this.NegAr; }
	public int getIncrAmount(){ return this.IncrAmount; }
	
	public int getNegAmount(){ return this.NegAmount; }
	...
	*/
	...
	protected void finalize(){
		this.destructorMultipleParArities();
	}
	protected void destructorMultipleParArities(){
	...
	public MultipleParArities getInverse(){
		return new MultipleParArities(this.IncrAr, this.NegAr,IncrAmount, NegAmount);
	}
	
	public int getA() {
		this.checkTheWholeBody();
		//return this.__theWholeBody__.getA();
		return this.__fixedRegistersAmount__ + this.IncrAr + this.NegAr;
	}
		...
		if(this.__theWholeBody__ == null){
			this.__theWholeBody__ = new RPP(){
				...
				*/
				private final RPP[] __subtasks__ = new RPP[]{ //
					new RPP(){ // BodyParamIncImpl // index: 0
						...
						public void b(int[] __x__, int __startIndex__, int __endIndex__) {
							int __arity__;
							int __repsAmount__ = 0 + (1*IncrAmount);
					...
					new RPP(){ // BodyParamNegImpl // index: 1
						...
						public void b(int[] __x__, int __startIndex__, int __endIndex__) {
							int __arity__;
							int __repsAmount__ = 0 + (1*NegAmount);
							while(__repsAmount__-->0){
							__arity__ = this.getA();
							while(__arity__-->0){
								this.__f__.b(__x__, __startIndex__ + __arity__, __startIndex__ + __arity__ + 1); // "1" because "f.getA()" will surely returns "1"
				...
				*/
				...
				public int getA() { return (0 + (1*NegAr) + (1*IncrAr)); }
