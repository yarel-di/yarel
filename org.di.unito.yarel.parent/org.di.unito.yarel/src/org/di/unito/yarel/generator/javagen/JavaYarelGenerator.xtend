...
//	@Inject extension YarelUtils yarelUtils
	...
	private def SwapGenerator(String packageName) {
		...
		*/
		public class Swap implements RPP {
			...
			public void b(BigInteger[] __x__, int __startIndex__, int __endIndex__) {
				BigInteger __temp__ = __x__[__startIndex__ + this.__firstIndex__];
	...
	private def SubBodyRunnerGenerator(String packageName) {
		 ...
		 * second parameter of {@link RPP#b(BigInteger[],int,int)}.
		*/
		public class SubBodyRunner implements Runnable{
			protected final int startingIndex;
			protected final BigInteger[] registers;
			protected final RPP subBody;
			
			protected SubBodyRunner(int startingIndex, RPP subBody, BigInteger[] registers){
...
//	}
		...
		public class «packageName.toFirstUpper»PlayWith {
			...
			public static void test«name.toFirstUpper»(){
				...
				final BigInteger[][] datasets = {
					new BigInteger[]{«FOR i : 0 ..< arity - 1»BigInteger.valueOf(«i+1»),«ENDFOR»BigInteger.valueOf(5)},
					new BigInteger[]{«FOR i : arity - 1 >..0»BigInteger.valueOf(«i+1»),«ENDFOR»BigInteger.valueOf(5)},
					«FOR i : 0..< baseValuesData.size »
						«val baseValue = baseValuesData.get(i)»
						new BigInteger[]{«FOR rep : 0 ..< arity SEPARATOR ", "» BigInteger.valueOf(«baseValue»)«ENDFOR»},
					«ENDFOR»
				};
	...
	private def collectArities(Model m) {
		for(var i = 0; i < m.elements.length; i++){
			...
			if(element.getContainerOfType(Declaration)!==null) { // if a declaration exists
				...
				var arity = 0;
				for(var j = 0; j < decl.types.size; j++) // every type component
					if (decl.types.get(j).value != 0)
						arity += decl.types.get(j).value // counts the explicit number of occurrences
					else  
						arity++ // counts +1
	...
	private def compile(Model model, Definition definition, boolean fwd) {
		...
		println("compiling model: " + model.name); // REMOVE ME BEFORE COMMIT
	...
	private def String compile(Body b, boolean  fwd, boolean[] hasParallelBlock) {
	  ...
	  switch (b) {
		...
		ParComp:{
			...
			if(!(parallelNodeIterator instanceof BodyId)){
				 // don't waste time on "identit(y/ies)"
				val currentBlockArity = YarelUtils.getArity( parallelNodeIterator);
				parallelSubBodies.addFirst(
					new BodySubblockParallel(
						startIndexOffset -= currentBlockArity,
			...
			if(parallelSubBodies.size == 1){
				...
				public void b(BigInteger[] __x__, int __startIndex__, int __endIndex__) {
					this.__f__.b(__x__,
						...
						__startIndex__ + («theOnlySubBlock.startIndexOffset.toString()») + this.__f__.getA(«/*theOnlySubBlock.bodyArity.toString*/»)
						);
				}
				'''
...
//			val funcArity = yarelUtils.getArityOfFunctionName(b, functionName)
		...
		BodyPerm:
			'''
				...
				public int getA() { return «b.permutation.indexes.length»; }
			'''
		BodyInv:
			'''
