...
import yarelcore.WrongArityException;

public class SortPreComparisonPart implements RPP {
	public SortPreComparisonPart(// arities:
			int K) {
		this.__fixedRegistersAmount__ = 4;
		if (K < 0) {
			throw new WrongArityException("The arity \"K\" cannot be negative: " + K);
		}
		this.K = K;
	}

	protected SortPreComparisonPart() {
		this(1);
	}
	...
	public InvSortPreComparisonPart getInverse() {
		return new InvSortPreComparisonPart(this.K);
	}
	...
	public void b(int[] __x__, int __startIndex__, int __endIndex__) {
		this.checkTheWholeBody();
		System.out.println("##pre-inverting the sortPreComparisonPart: " + Arrays.toString(__x__));
	...
	protected void checkTheWholeBody() {
		if (this.__theWholeBody__ == null) {
			this.__theWholeBody__ = new RPP() {
				private final RPP[] __steps__ = new RPP[] { new RPP() { // ParCompImpl // index: 0
					private RPP __f__ = new RPP() {
						...
						public void b(int[] __x__, int __startIndex__, int __endIndex__) {
							...
							while (__repsAmount__-- > 0) {
								__arity__ = this.getA();
								while (__arity__-- > 0) {
									this.__f__.b(__x__, __startIndex__ + __arity__, __startIndex__ + __arity__ + 1); // "1"
																														// because
																														// "f.getA()"
																														// will
																														// surely
					...
					@Override
					public void b(int[] __x__, int __startIndex__, int __endIndex__) {
						this.__f__.b(__x__, __startIndex__ + 2 + (1 * K),
						...
						new RPP() { // BodyParamPermImpl // index: 1
							...
							public void b(int[] __x__, int __startIndex__, int __endIndex__) {
								...
								if (__indexToWithdraw__ < 0) {
									__indexToWithdraw__ = -__indexToWithdraw__;
								}
							...
							private RPP __f__ = new RPP() {
								...
								public void b(int[] __x__, int __startIndex__, int __endIndex__) {
									...
									while (__repsAmount__-- > 0) {
										__arity__ = this.getA();
										while (__arity__-- > 0) {
											this.__f__.b(__x__, __startIndex__ + __arity__,
													__startIndex__ + __arity__ + 1); // "1" because "f.getA()" will
																						// surely returns "1"
										}
									}
								}
